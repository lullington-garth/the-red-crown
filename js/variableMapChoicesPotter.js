...
export function resolvePotterVariable(path, playerStats) {
    ...
    switch (path) {
        ...
        case "potterOutside":
            if (wizard === "yellow") {
                return `Sitting at a junction is a small potter’s cottage, its whitewashed walls stained with dust from the nearby roads. Thin trails of grey smoke curl from a crooked chimney, and rows of clay pots dry upon wooden shelves beneath the windows. A high level of craftsmanship has gone into those pots. You can just make out the steady clack of a turning wheel as it drifts out through the open doorway, carrying with it the warm smell of clay and kiln fire.`;
            }
            if (wizard === "green") {
            ...
            if (wizard === "red") {
                return `At the next junction stands a crooked pet shop with warped black timbers and smudged, yellowing windows. Scratches, barks, snorts, howls, and chirping come from inside, and wicked-looking cages hang beneath the awning overhead, empty for now and swaying gently in the wind. Painted above the door in rough red lettering are the words: Mystical Creatures Bought and Sold.`;
            }
...
At the rear of the shop stands a tall iron cabinet secured with a thick brass lock. Resting upon a cushion inside is a crystal skull no larger than a clenched fist. Pale light swirls like drifting mist within it.

‘The Whispering Skull,’ says the man, following your gaze. ‘While you hold it, you cannot die. Far too valuable to sell, I'm afraid.’`;
            }
            if (wizard === "blue") {
...
Behind the counter stands a thin old toymaker in a patchwork waistcoat, his silver spectacles perched low on the end of his crooked nose. He looks up from a half-finished marionette. He smiles pleasantly and says, ‘Welcome, traveller. Mind the soldier by your foot. He bites.’

Around the old man’s neck hangs a delicate golden music box no larger than a pocket watch, engraved with tiny stars and moons. You have seen one of those before. It's a Lullaby Box. When wound, the beautiful sound will send all but the wearer to sleep. They are extremely rare and immensely powerful. You could bring down a dragon with a Lullaby Box if it were within earshot.

‘Not for sale,’ says the shopkeeper, noticing your gaze. He tucks the little box into his shirt.`;
            }
            if (wizard === "red") {
                return `The stench inside the shop is appalling. The cramped room is hot, damp, and filthy, filled with a cacophony of animal sounds: growling, chittering, sniffing, and chirping, but somehow all of it unhappy. Cages line every wall from floor to ceiling, holding creatures whose names you can hardly guess. Hairless cats with too many eyes stare at you, while lizards with glowing tongues spit black pitch, and black-feathered birds mutter softly amongst their many heads.

Behind the counter stands the shopkeeper, a tall, sly-looking man in a stained leather coat. His greasy hair hangs loose about his face, and his calloused hands usher a vicious-looking rodent into a corked jar. He glances up at you.

‘Did the matron send you?’ he rasps. When you indicate that she did not, he loses interest in you.

At the end of the counter, curled up in a cage too small for her, lies what you recognise to be a Moonveil Fox. Those to whom the fox feels it owes a debt will be granted moon magic. Very rare and very powerful.`;
            } 
...
‘The Goddess Blessing,’ he says in a hushed tone. ‘You will never be without magic with this little ring. Its magic is faint, but for twenty Gold Pieces I can rekindle it.’`;
            }
            if (wizard === "blue") {
...
‘Ever considered a familiar?’ he says. ‘This one's a clever little devil.’ He pulls down a jar with a tiny black imp demon inside it. ‘Knows spells, this one. Let him sleep around your neck and he'll fight for you too. Furiously loyal once bound. Twenty Gold Pieces and he’s yours, and I'll throw in the binding for free.’`;
            }

        case "potterSpecial":
            if (wizard === "yellow") {
                return `Would you like to ask the potter to make a protective charm pendant for you?`;
            }
            if (wizard === "green") {
                return `Would you like to ask the antique dealer to rekindle the magical ring for you?`;
            }
            if (wizard === "blue") {
                return `Would you like to ask the toy shop owner to enchant a lucky coin for you?`;
            }
            if (wizard === "red") {
                return `Would you like to ask the shopkeeper to bind the familiar to you?`;
            }        
        ...
        case "potterAttack":
            ...
            if (wizard === "green") {
                return `Attack the antique dealer and make for the skull?`;
            }
            if (wizard === "blue") {
                return `Attack the toy shop owner and take the Lullaby Box?`;
            }
            if (wizard === "red") {
                return `Attack the shopkeeper and free the fox?`;
            }
        ...
        case "potterWares":
            ...
            if (wizard === "green") {
                return `You move slowly about the crowded shop, examining the clutter of objects on display. There are some real finds dotted around the shop, but for today your needs are modest. There are a few items you consider, though.`;
            }
            if (wizard === "blue") {
            ...
            if (wizard === "red") {
                return `You look around the cramped pet shop, trying not to get too close to the cages. While the animals are of no real interest to you, the shopkeeper does have a few other items you feel are worth a look.`;
            }  
        ...
        case "potterFight":
            ...
            if (wizard === "red") {
                return `You draw your weapon and charge the shopkeeper, hoping to free the Moonveil Fox and gain its loyalty before the man knows what's happening, but he seems ready for you. He snatches up a wicked-looking hook, clacking it against the cages. The animals around the shop go wild.`;
            }

        case "potterCharm":
            if (wizard === "yellow") {
                return `You hand over the Gold Pieces and wait beside the warm kiln, enjoying the potter's company as he carefully shapes the clay with steady hands. A fair time later, the ward is ready and he presents you with a delicate pendant etched with protective symbols.
...
The Jester's Gambit`;
            ...
            if (wizard === "red") {
                return `You hand over the Gold Pieces and wait while the shopkeeper performs the binding ritual. The imp hisses and spits, cursing all the while as silver smoke coils around it, forming tight chains. When the creature is bound, the chains vanish and the imp scurries up your arm.
...
Imp Demon`;
        ...
        case "potterRegret":
            ...
            if (wizard === "green") {
                return `The antique dealer lies dead among the shattered relics of his shop. The crystal skull lies smashed beside him, the cabinet having been upended in the struggle. A strange pale light leaks from the shattered skull, fading into nothing as you watch. You look around the ruined shop and realise, with a stab of shame, that greed has led you to kill a good man and destroy something ancient and irreplaceable.`;
            }
            if (wizard === "blue") {
                return `The old toymaker lies motionless upon the shop floor, surrounded by shattered toys and splintered wood. He has fallen onto the golden music box, crushing it beyond repair. You look down at the macabre scene. What have you done? Led by greed, you have killed a good man and destroyed a priceless artefact.`;
            }
            if (wizard === "red") {
                return `The shopkeeper lies dead among broken cages. Terrified creatures screech, flutter, and cry out while others flee into the streets outside. You look to the fox, but it is dead. It must have been struck by a blow as you fought to free it. You have killed a man and destroyed something rare and precious.`;
            }
