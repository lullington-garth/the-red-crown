...
case "bathhouseName":
    ...
    if (wizard === "blue") {
        return `Buns & Beans Wholefood Café`;
    }
    if (wizard === "red") {
...
‘Ready, sir?’`;
    ...
    if (wizard === "red") {
        return `Max looks thrilled. He cracks his knuckles and readies himself to fight as the patrons form a ring around the two of you, chanting 'FIGHT! FIGHT! FIGHT! FIGHT!'`;
    }
...
case "service2Name":
    ...
    if (wizard === "red") {
        ...
        case "bathhouseSkin":
            ...
            if (wizard === "red") {
                return `With the cottage behind you, you continue south along Brookmeadow Lane. Before long, you begin to hear shouts and roars echoing from behind the trees. Rounding the bend in the road, you discover a large open training yard surrounded by rough timber fencing. Massive stones, wagon wheels and iron chains lie scattered about the muddy ground, while thick wooden beams support dangling ropes and punching sacks stuffed with straw. A large sign swings over the entrance:
...
case "bathhouseEntrance":
    ...
    if (wizard === "red") {
        return `You look around what you decide must be some sort of gym for huge creatures. A Minotaur strains beneath an enormous boulder while two Ogres repeatedly strike one another across the face to loud applause. Nearby, a heavily muscled Goblin hangs upside down from iron hooks whilst furiously lifting bricks tied to his ankles. Everywhere there is grunting, screaming and the sound of testosterone-fuelled exertion.
                ...
                At the centre of the yard stands an enormous bald man whose belly hangs heavily over his iron-studded belt. Sergeant's stripes are tattooed across one massive upper arm. Veins bulge from his neck as he leans over what appears to be a large rock.
    ...
    case "service1":
        ...
        if (wizard === "blue") {
            ...
            It is disgusting — dry, grainy, claggy, foul beyond belief. It clings to your mouth like dust and carries a powerful smell not unlike warm dung. Fighting every instinct to vomit, you desperately gulp some water just to force the mouthful down.

            The waiter looks expectant.

            ‘Good,’ you nod weakly.

            The waiter seems delighted. ‘He like it,’ he says to the unicorn, then scurries off.
    ...
    case "complain":
        if (wizard === "green") {
            return `You drag yourself to the reception desk. WHAT WAS THAT! You angrily complain about the Total Sensory Renewal. The grey-skinned receptionist puts her magazine down and fixes you with a bored-looking expression. She puffs out a plume of foul-smelling smoke in your face.
...
`;
        ...
        if (wizard === "yellow") {
            return `You drag yourself to the reception desk. ‘WHAT WAS THAT?’ you splutter before complaining bitterly about the Thousand Veils Immersion. The fawn tilts her head sympathetically.
...
`;
    ...
    case "complain2":
        if (wizard === "green") {
            return `You stagger back to the reception desk, your head still spinning. You slump against the desk, anger the only thing keeping you upright, and protest furiously about the Breathless Clarity Ritual and your missing gold.

            The receptionist glances up at you from her magazine with a look of complete disinterest.

            ‘You were breathless, weren’t you?’ she says flatly. She then rings a small bell. ‘Make way for the next customer.’`;
        }
        if (wizard === "yellow") {
            return `You stagger back to the reception desk, your head still spinning. You slump against the desk, anger the only thing keeping you upright, and protest furiously about the Silent Passing Repose. The fawn tilts her head sympathetically.
 ...
 case "jarsBathhouse":
    ...
    if (wizard === "yellow") {
        return `Nobody seems too bothered about the fact that you have just slain the receptionist, so you decide to chance your luck and take a look behind the reception counter. The few pamphlets and stationery items are of no interest to you, but there are several jars of cream neatly stacked at the back of the shelf. Their labels proclaim them to be:
    ...
    if (wizard === "blue") {
        return `You look through the storage boxes. It's mostly useless to you, but you do find a bag of acidic-smelling mushrooms marked Healing Soother Spores. They sting to the touch, but the packet says they are great healers when eaten.`;
    }
    if (wizard === "red") {
...
case "applyJarBathhouse":
    ...
    if (wizard === "blue") {
        return `They sting your fingers, lips and tongue, burning your throat as you try to swallow them. This was a mistake. Your insides tingle, and then something warm burns inside you. Seconds later, you are amazed as the burning passes and you feel the healing properties taking effect.
...
case "giftShop":
    ...
    if (wizard === "yellow") {
        return `The lady behind the marble gift shop counter is equally as courteous as the receptionist.
    ...
    if (wizard === "blue") {
        return `The small man hands you over to an equally small female set up in front of a table covered in a gingham cloth.
                ...
                You notice they seem to be leaving scorch marks on the cloth and the lady is wearing thick hide gloves to handle them.`;
    }
    if (wizard === "red") {
        return `You ask where the gift shop is. Max pulls a luminous green energy drink from his belt and thrusts it at you.
